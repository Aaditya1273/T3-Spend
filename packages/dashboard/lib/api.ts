...
const BASE = process.env.NEXT_PUBLIC_T3SPEND_API ?? "http://localhost:4070/api";
...
export const api = {
  // --- T3N DID binding & issuance ---
  bindCardToDID: (cardId: string, agentDid: string, agentAddress?: string) =>
    call<{ card_id: string; agent_did: string; agent_address: string; bound: boolean }>(`/cards/${cardId}/bind-did`, {
      method: "POST",
      body: JSON.stringify({ agent_did: agentDid, ...(agentAddress ? { agent_address: agentAddress } : {}) }),
    }),
  unbindCardDID: (cardId: string) =>
    call<{ card_id: string; bound: boolean }>(`/cards/${cardId}/unbind-did`, { method: "POST", body: "{}" }),
  /** Issue a card bound to an agent's did:t3n identifier (server-signed, admin only). */
  issueCardForDID: (name: string, terms: CardTermsInput, agentDid: string, agentAddress?: string) =>
    call<{ card_id: string; card_url: string; agent_did: string; agent_address: string; terms: CardTermsInput }>(
      "/cards/issue-for-did",
      {
        method: "POST",
        body: JSON.stringify({ name, terms, agent_did: agentDid, ...(agentAddress ? { agent_address: agentAddress } : {}) }),
      },
    ),
