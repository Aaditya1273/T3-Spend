  ...
  return (
      ...
      <main className="docbody">
        <div className="docwrap">
          ...
          <header className="dochero">
            ...
            <h1>Documentation</h1>
            <p className="docsub">
              T3 Spend issues scoped, revocable spending cards from your wallet. Any agent plugs one in over MCP and
          ...
          <Section id="overview" title="Overview">
            <p className="docp">
              ...
              wallet loses both your custody and your limits.              T3 Spend takes the model the card industry settled on
   ...
   └── `}<b>card</b>{`   `}<span className="mut">$25 / week · expires Jul 6</span>{`        ← root delegation, signed by you
        ...
        └── `}<b>sub-card</b>{`   `}<span className="mut">$1 / week · one merchant</span>{`   ← redelegation, narrower terms
            ...
            <p className="docp">
              T3 Spend runs on <b>Base mainnet</b> with real USDC. The only simulated leg is the Visa rail (Stripe
...
droid     mcp add remit https://<host>/c/<secret>/mcp --type http`} />
          ...
          <Section id="rails" title="Payment Rails">
            ...
            <p className="docp">
              ...
              (gasless, fee in USDC).              T3 Spend also ships the first ERC-7710 x402 facilitator
            ...
            <p className="docp">
              ...
              authorized,              Stripe calls T3 Spend&apos;s real-time auth webhook, which answers approve/decline from the
          ...
          <Section id="security" title="Security">
            <ul className="docul">
              <li className="docli">
                ...
                account menu at any time (through Privy&apos;s secure modal, rendered in a separate-domain              iframe T3 Spend never reads) and walk away to any client.
              </li>
              <li className="docli">
          ...
          <Section id="api" title="API Reference">
            ...
            <ul className="docul">
              <li className="docli">
                <b>Admin</b> (<code>Authorization: Bearer &lt;T3SPEND_ADMIN_TOKEN&gt;</code>): full access, server-side
...
bun run --cwd packages/dashboard dev   # dashboard on :4071`} />
            ...
            <Table
              head={["Var", "Purpose"]}
              rows={[
                [<code key="e">T3SPEND_MASTER_KEY</code>, "32-byte hex key; encrypts agent keys + card secrets at rest"],
                [<code key="e">T3SPEND_ADMIN_TOKEN</code>, "Ops bearer token for the management API (server-side only)"],
                [<code key="e">T3SPEND_PRIVY_APP_ID</code>, "Dashboard lane: enables per-user Privy auth against the app JWKS"],
              ]}
            />
            ...
            <Table
              head={["Var", "Purpose"]}
              rows={[
                [<code key="e">T3SPEND_PUBLIC_MCP_BASE</code>, "Public origin for card URLs (also arms the MCP Host allowlist)"],
                [<code key="e">T3SPEND_CORS_ORIGINS</code>, "Comma-separated allowed origins for the API + shop"],
                [<code key="e">STRIPE_SECRET_KEY</code>, "Stripe TEST-mode key (sk_test_/rk_test_ only); enables the fiat leg"],
                [<code key="e">T3SPEND_STRIPE_WEBHOOK_SECRET</code>, "Real-time auth webhook secret; unset = fiat leg disabled (503)"],
                [<code key="e">T3SPEND_FIAT_SETTLEMENT</code>, "1 = approved Visa charges settle on-chain as real USDC"],
                [<code key="e">VENICE_API_KEY · VENICE_MODEL</code>, "Enables /cards/compile; pin the model id"],
                [<code key="e">T3SPEND_DASHBOARD_BASE</code>, "Dashboard origin hosting the OAuth consent page"],
                [<code key="e">T3SPEND_RPC_URL · NEXT_PUBLIC_BASE_RPC</code>, "Base RPC for server + client reads (default mainnet.base.org)"],
                [<code key="e">T3SPEND_DB_PATH</code>, "SQLite path (default .dev/t3spend.sqlite)"],
                [<code key="e">T3SPEND_ALLOWED_HOSTS</code>, "Extra Host headers accepted on the MCP endpoint (e.g. a platform fallback domain)"],
                [<code key="e">BASESCAN_API_KEY</code>, "Verified-contract labels from Basescan when resolving compiled drafts"],
                [<code key="e">T3SPEND_TRUST_PROXY_HOPS</code>, "Trusted proxy hops for client-IP rate limiting (default 1 = Railway edge)"],
                [<code key="e">T3SPEND_MCP_RATE_LIMIT · T3SPEND_MCP_BAD_SECRET_LIMIT</code>, "Per-card and per-IP-bad-secret request ceilings per minute (240 / 30)"],
                [<code key="e">T3SPEND_OAUTH_ACCESS_TTL · T3SPEND_OAUTH_REFRESH_TTL</code>, "OAuth access / refresh token lifetimes in seconds (3600 / 2592000)"],
                [<code key="e">T3SPEND_OAUTH_REDIRECT_HOSTS</code>, "If set, restricts OAuth https redirect-URI hosts to this allowlist (recommended in prod)"],
              ]}
            />
