...
import { IconCheck, IconCopy, IconRevoke, IconShield, isDead, shortHex } from "./ui";
...
// ---------------------------------------------------------------------------
// T3N Connect: bind a card to a Terminal 3 DID so the agent can authenticate
// via T3N session instead of a bearer URL. The agent must first authenticate
// to T3N to get its did:t3n:<hex>, then paste it here.
// ---------------------------------------------------------------------------

export function T3NConnect({
  cardId,
  t3nBound,
  agentDid,
  onBound,
  ...
  const bind = useCallback(async () => {
    if (!did.trim() || busy) return;
    setErr(null);
    setBusy(true);
    try {
      await api.bindCardToDID(cardId, did.trim());
      setDid("");
      onBound();
    } catch (e) {
      setErr(e instanceof Error ? e.message : String(e));
  ...
  }, [did, busy, cardId, onBound]);

  const unbind = useCallback(async () => {
    setErr(null);
    setBusy(true);
    try {
      await api.unbindCardDID(cardId);
      onBound();
    } catch (e) {
      setErr(e instanceof Error ? e.message : String(e));
    } finally {
  ...
  if (t3nBound && agentDid) {
    return (
      <div className="t3n-bound">
        <div className="t3n-row">
          <IconShield />
          <span className="t3n-did" title={agentDid}>
            {shortHex("did:t3n:" + agentDid.slice(8), 8, 4) || agentDid}
          </span>
          <span className="pill live" style={{ fontSize: 10, margin: 0 }}>
            <b />
            T3N
          </span>
        </div>
        <button className="vpill stay" onClick={unbind} disabled={busy}>
          Unbind
        </button>
  ...
  return (
    <div className="t3n-connect">
      <div className="t3n-row">
        <IconShield />
        <span style={{ fontSize: 12, color: "var(--body)" }}>
          Connect via Terminal 3 Agent DID
        </span>
      </div>
      <div className="t3n-input-row">
        <input
          ...
          placeholder="did:t3n:... (paste the agent's DID)"
          className="t3n-input"
          disabled={busy}
        />
        <button className="dbtn quiet" onClick={bind} disabled={busy || !did.trim()}>
          {busy ? "Binding…" : "Bind"}
        </button>
      </div>
      {err && <p className="err" style={{ fontSize: 12 }}>{err}</p>}
      <p className="bchint">
        The agent authenticates via T3N session — no bearer URL needed
      </p>
    </div>
  );
}
