...
import { ChipDots, Guilloche, IconClose, IconShield, isDead, periodLabel, shortHex } from "./components/ui";
...
function IssueModal({
  ...
  // T3N DID binding
  const [t3nDid, setT3nDid] = useState("");
  const [t3nMode, setT3nMode] = useState(false);
  // Auto-disable sub-cards when T3N mode is active (no K_agent to sign sub-delegations)
  useEffect(() => {
    if (t3nMode) setSubcards(false);
  }, [t3nMode]);
  // pay lane
  ...
  async function issue() {
    ...
    setBusy(true);
    try {
      if (t3nMode && t3nDid.trim()) {
        // T3N DID-backed issuance: server-signed, delegate = agent's DID address
        const res = await api.issueCardForDID(name, terms, t3nDid.trim());
        setIssuedUrl(res.card_url);
        onIssued();
      } else {
        // Standard client-signed issuance: prepare -> sign -> finalize
        ...
        {issuedUrl ? (
          ...
          <>
            ...
            <AnimatePresence initial={false}>
              {editOpen && (
                ...
                <div className="csec">
                  ...
                  <div className="csecrow" style={{ marginTop: 8 }}>
                    {field(
                      "T3N Agent DID",
                      <div className="t3n-input-row">
                        <input
                          value={t3nDid}
                          onChange={(e) => setT3nDid(e.target.value)}
                          placeholder="did:t3n:…"
                          style={{ width: 220 }}
                          data-testid="composer-t3ndid"
                        />
                        <span
                          className={`t3n-toggle${t3nMode ? " active" : ""}`}
                          onClick={() => setT3nMode((v) => !v)}
                          title={t3nMode ? "Disable T3N binding" : "Bind card to agent DID"}
                          style={{
                            cursor: "pointer",
                            fontSize: 11,
                            padding: "4px 8px",
                            borderRadius: 6,
                            background: t3nMode ? "var(--accent)" : "var(--surface)",
                            color: t3nMode ? "#fff" : "var(--body)",
                            border: `1px solid ${t3nMode ? "var(--accent)" : "var(--line)"}`,
                            whiteSpace: "nowrap",
                          }}
                        >
                          <IconShield />
                          {t3nMode ? "T3N Active" : "T3N"}
                        </span>
                      </div>
                    )}
                  </div>
            ...
            <div className="mfoot">
              ...
              {t3nMode && t3nDid.trim() ? (
                <button className="dbtn" onClick={issue} disabled={busy} data-testid="composer-issue-t3n">
                  {busy ? "Issuing…" : "Issue T3N Card"}
                </button>
              ) : (
