...
// Master key lives OUTSIDE the DB (env T3SPEND_MASTER_KEY, 32-byte hex; KMS post-hackathon).
...
async function masterKey(): Promise<CryptoKey> {
  const raw = process.env.T3SPEND_MASTER_KEY;
  if (!raw || !/^(0x)?[0-9a-fA-F]{64}$/.test(raw)) {
    throw new EngineError(
      "custody",
      "T3SPEND_MASTER_KEY must be set to 32-byte hex (generate: openssl rand -hex 32)",
    );
  }
