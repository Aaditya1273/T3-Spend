...
export type CardRow = {
  ...
  /** T3N agent DID bound to this card (did:t3n:<hex>); null = not T3N-bound */
  t3n_did: string | null;
  /** JSON-serialized T3NBinding for the agent DID */
  t3n_binding_json: string | null;
  status: CardStatus;
  created_at: number;
};
...
export class Store {
  ...
  private migrate() {
    ...
    // additive migration for T3N DID binding columns
    const cardCols2 = this.db.query(`PRAGMA table_info(cards)`).all() as Array<{ name: string }>;
    if (!cardCols2.some((c) => c.name === "t3n_did")) {
      this.db.exec(`ALTER TABLE cards ADD COLUMN t3n_did TEXT`);
      this.db.exec(`ALTER TABLE cards ADD COLUMN t3n_binding_json TEXT`);
    }
    this.db.exec(
      `CREATE INDEX IF NOT EXISTS idx_cards_t3n_did ON cards(t3n_did) WHERE t3n_did IS NOT NULL`,
    );
    ...
    this.db
      .query(
        `INSERT INTO cards (id, user_id, parent_card_id, name, secret_hash, secret_enc, terms_json, kind,
                            compiled_json, delegation_json, k_agent_enc, k_agent_address,
                            t3n_did, t3n_binding_json, status, created_at)
         VALUES ($id, $user, $parent, $name, $hash, $senc, $terms, $kind, $compiled, $delegation, $kenc, $kaddr,
                 $t3nDid, $t3nBind, $status, $created)`,
      )
      .run({
        $id: c.id,
        $user: c.user_id,
        ...
        $t3nDid: c.t3n_did ?? null,
        $t3nBind: c.t3n_binding_json ?? null,
        $status: c.status,
  ...
  private rowToCard(r: Record<string, unknown> | null): CardRow | null {
    ...
    return {
      ...
      t3n_did: (r.t3n_did as string) ?? null,
      t3n_binding_json: (r.t3n_binding_json as string) ?? null,
  ...
  getCardByDID(did: string): CardRow | null {
    return this.rowToCard(
      this.db.query(`SELECT * FROM cards WHERE t3n_did = $d COLLATE NOCASE`).get({ $d: did }) as never,
    );
  }

  listAllCards(): CardRow[] {
    const rows = this.db.query(`SELECT * FROM cards ORDER BY created_at`).all() as never[];
  ...
  bindCardToDID(cardId: string, did: string, bindingJson: string): void {
    this.db
      .query(`UPDATE cards SET t3n_did = $d, t3n_binding_json = $b WHERE id = $id`)
      .run({ $d: did, $b: bindingJson, $id: cardId });
  }

  unbindCardDID(cardId: string): void {
    this.db
      .query(`UPDATE cards SET t3n_did = NULL, t3n_binding_json = NULL WHERE id = $id`)
      .run({ $id: cardId });
  }
