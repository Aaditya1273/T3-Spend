...
export async function spend(deps: SpendDeps, cardId: string, req: SpendRequest): Promise<Receipt> {
  ...
  if (req.settleChargeId) {
    ...
    if (row.status === "confirmed" || row.status === "failed" || row.status === "settlement_unconfirmed") {
      ...
      return receiptFromCharge(deps, cardId, row.status, row.tx_hash, row.to_addr ?? FEE_COLLECTOR, row.amount_atoms, row.fee_atoms, now, row.memo ?? undefined, row.id);
    }
    if (row.request_id !== null) {
      ...
      return receiptFromCharge(deps, cardId, "pending", row.tx_hash, row.to_addr ?? FEE_COLLECTOR, row.amount_atoms, row.fee_atoms, now, row.memo ?? undefined, row.id);
    }
  ...
  if (req.idempotencyKey && !settleRow) {
    ...
    if (existing) {
      ...
      if (existing.status === "failed" && existing.request_id === null) {
        ...
        return receiptFromCharge(deps, cardId, existing.status, existing.tx_hash, existing.to_addr ?? req.to ?? FEE_COLLECTOR, existing.amount_atoms, existing.fee_atoms, now, existing.memo ?? undefined, existing.id);
      }
    }
  }
  ...
  for (let attempt = 0; attempt < ESTIMATE_RETRIES; attempt++) {
    ...
    if (confirmation.status === "confirmed") {
      ...
      return receiptFromCharge(deps, cardId, "confirmed", confirmation.txHash, req.to ?? FEE_COLLECTOR, amountAtoms, feeAtoms, now, req.memo, chargeId);
    }
    ...
    return receiptFromCharge(deps, cardId, "pending", confirmation.txHash, req.to ?? FEE_COLLECTOR, amountAtoms, feeAtoms, now, req.memo, chargeId);
  }
