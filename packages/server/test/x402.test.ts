...
beforeAll(() => {
  process.env.T3SPEND_MASTER_KEY = "e".repeat(64);
  process.env.T3SPEND_PAID_FETCH_ALLOW_LOCAL = "1";
  ...
  process.env.T3SPEND_FACILITATOR_BASE = `${base}/facilitator`;
  store.upsertUser({ id: "u-x402", address: user.address });
});

afterAll(() => {
  server.stop(true);
  delete process.env.T3SPEND_PAID_FETCH_ALLOW_LOCAL;
});
...
describe("the full paid_fetch loop", () => {
  ...
  test("max_price refusal; non-402 passthrough; SSRF guard", async () => {
    ...
    process.env.T3SPEND_PAID_FETCH_ALLOW_LOCAL = "";
