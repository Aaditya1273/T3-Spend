...
//   - ADMIN (ops): Bearer T3SPEND_ADMIN_TOKEN · full access, server-side curl/scripts.
...
import { resolveDIDToAddress } from "../mcp/t3n-auth";
...
export function apiRoutes(deps: AppDeps, oauth: OAuthStore): Hono<{ Variables: { auth: AuthCtx } }> {
  ...
  app.use("*", async (c: Context<{ Variables: { auth: AuthCtx } }>, next: Next) => {
    if (!deps.adminToken && !deps.verifyPrivyToken) {
      return c.json({ error: "api disabled (no T3SPEND_ADMIN_TOKEN and no T3SPEND_PRIVY_APP_ID)" }, 503);
    }
  ...
  // ---- T3N DID-backed issuance ----
  // Issue a card bound to an agent's did:t3n identifier. The delegate is the
  // agent's DID-resolved address — no server-custodied K_agent.
  // Two auth lanes:
  //   ADMIN: server-signed with T3SPEND_DEV_USER_PK; userId from body.
  //   PRIVY: client-signed through the standard prepare→sign→finalize ceremony,
  //          but pinned to the user's own wallet; always server-signed here
  //          because the delegate address must be set to the agent's DID key
  //          (not a K_agent). The user still signs the root delegation as the
  //          delegator (their own wallet), so no admin key exposure needed.
  app.post("/cards/issue-for-did", (c) =>
    handle(c, async () => {
      if (!deps.t3n?.enabled) throw new EngineError("api", "T3N integration disabled");
      ...
      if (!body.agent_did || !isValidT3NDID(body.agent_did)) {
        throw new RefusalError("invalid_terms", "valid did:t3n identifier required");
      }

      // Auth: admin (dev signer) or privy (pinned to onboarded wallet)
      let userSigner;
      let userId: string;
      if (c.get("auth").kind === "admin") {
        if (!deps.userSigner) throw new EngineError("api", "no dev signer configured (T3SPEND_DEV_USER_PK)");
        userSigner = deps.userSigner;
        userId = normUserId(body.userId ?? deps.userSigner.address);
      } else if (c.get("auth").kind === "privy") {
        const user = boundUser(c);
        if (!deps.userSigner) throw new EngineError("api", "no signer configured (T3SPEND_DEV_USER_PK)");
        userSigner = deps.userSigner;
        userId = user.id;
      } else {
      ...
      // Resolve or accept the agent's address
      let agentAddress = body.agent_address;
      if (!agentAddress || !isAddress(agentAddress)) {
        const resolved = await resolveDIDToAddress(body.agent_did, deps.store);
        if (!resolved) {
          throw new RefusalError("invalid_terms", "could not resolve DID to address; provide agent_address");
        }
        agentAddress = resolved;
      }

      const issued = await issueCardForDID(
        { store: deps.store, userSigner },
        { userId, name: body.name, terms: body.terms, agentDid: body.agent_did, agentAddress },
      );
      return {
        card_id: issued.cardId,
        card_url: cardUrl(issued.secret),
  ...
  // ---- T3N DID binding ----
  // Bind a card to an agent's did:t3n identifier. After binding, the agent
  // can authenticate to the MCP server via T3N DID instead of a bearer URL.
  // Only the card owner or admin may bind.
  app.post("/cards/:id/bind-did", (c) =>
    handle(c, async () => {
      const card = ownedCard(c, c.req.param("id"));
      const body = (await c.req.json()) as { agent_did?: string; agent_address?: Address };
      if (!body.agent_did || !isValidT3NDID(body.agent_did)) {
        throw new RefusalError("invalid_terms", "valid did:t3n identifier required");
      }
      // Resolve the DID to an address if not provided
      let addr = body.agent_address ?? null;
      if (!addr) {
        addr = await resolveDIDToAddress(body.agent_did, deps.store);
      }
      if (!addr || !isAddress(addr)) {
        throw new RefusalError("invalid_terms", "could not resolve DID to address");
      }
      const binding = createBinding(body.agent_did, addr);
      deps.store.bindCardToDID(card.id, body.agent_did, serializeBinding(binding));
      return { card_id: card.id, agent_did: body.agent_did, agent_address: addr, bound: true };
    }),
  );

  // Unbind a card from its T3N DID (disables T3N auth for this card).
  app.post("/cards/:id/unbind-did", (c) =>
    handle(c, async () => {
      const card = ownedCard(c, c.req.param("id"));
      deps.store.unbindCardDID(card.id);
      return { card_id: card.id, bound: false };
    }),
  );
