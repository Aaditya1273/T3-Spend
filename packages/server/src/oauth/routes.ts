...
export function publicBase(): string {
  const raw = process.env.T3SPEND_PUBLIC_MCP_BASE ?? `http://localhost:${process.env.PORT ?? 4070}`;
  return raw.replace(/\/+$/, "");
}
...
const dashboardBase = (): string =>
  (process.env.T3SPEND_DASHBOARD_BASE ?? "http://localhost:4071").replace(/\/+$/, "");
 ...
 *  - https: any host (optionally restricted via T3SPEND_OAUTH_REDIRECT_HOSTS),
...
function redirectUriAllowed(uri: string): boolean {
  ...
  if (url.protocol === "https:") {
    const restrict = (process.env.T3SPEND_OAUTH_REDIRECT_HOSTS ?? "").split(",").map((s) => s.trim()).filter(Boolean);
...
export const acceptedResources = (): string[] => {
  const extra = (process.env.T3SPEND_OAUTH_ACCEPTED_RESOURCES ?? "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
...
export function oauthRoutes(deps: AppDeps, oauth: OAuthStore): Hono {
  ...
  const accessTtl = () => envInt("T3SPEND_OAUTH_ACCESS_TTL", 3600);
  const refreshTtl = () => envInt("T3SPEND_OAUTH_REFRESH_TTL", 30 * 86_400);

  const registerLimit = new RateLimiter(envInt("T3SPEND_OAUTH_REGISTER_LIMIT", 10), 60_000);
  const authorizeLimit = new RateLimiter(envInt("T3SPEND_OAUTH_AUTHORIZE_LIMIT", 30), 60_000);
  const tokenLimit = new RateLimiter(envInt("T3SPEND_OAUTH_TOKEN_LIMIT", 60), 60_000);
